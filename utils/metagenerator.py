...
from .pinata_uploader import upload_to_pinata  # Assuming you wrote this
# utils/metadata_generator.py
...
def upload_metadata(encrypted_file_path, api_key, secret_key):
    hash_value = get_sha256(encrypted_file_path)
    ipfs_url = upload_to_pinata(encrypted_file_path, api_key, secret_key)
    if not ipfs_url:
        raise Exception("Encrypted file upload failed.")

    metadata_file = create_metadata_json(ipfs_url, hash_value)
    metadata_ipfs_url = upload_to_pinata(metadata_file, api_key, secret_key)
    return metadata_ipfs_url